...
public class Solution {
	...
	float[] sumSliceSize2 = new float[A.length - 1];
	float[] sumSliceSize3 = new float[A.length - 2];

	int startingPosition = A.length;
	float minimalAverage = Integer.MAX_VALUE;

	for (int i = 0; i < A.length - 1; i++) {
	    sumSliceSize2[i] = ((float) (A[i] + A[i + 1])) / 2;
	    if (i + 2 < A.length) {
		sumSliceSize3[i] = ((float) (A[i] + A[i + 1] + A[i + 2])) / 3;
	    }

	    if (i < sumSliceSize2.length && sumSliceSize2[i] < minimalAverage) {
		minimalAverage = sumSliceSize2[i];
		startingPosition = i;
	    }
	    if (i < sumSliceSize3.length && sumSliceSize3[i] < minimalAverage) {
		minimalAverage = sumSliceSize3[i];
		startingPosition = i;
	    }
	}
