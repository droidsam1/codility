...
public class Solution {
	...
	assert (A < Integer.MAX_VALUE && A >= 0);
	assert (B < Integer.MAX_VALUE && B >= 0);

	int upperDivisor = A / K;
	int lowerDivisor = B / K;
	int numberOfIntegersWithModEqualsZero = lowerDivisor - upperDivisor;

	if (A % K == 0) {
	    numberOfIntegersWithModEqualsZero++;
	}
	return numberOfIntegersWithModEqualsZero;
